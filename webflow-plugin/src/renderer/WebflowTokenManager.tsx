import { type JSXElement, createSignal, onMount, For, Show } from "solid-js";
...
type PluginMessage =
  ...
  | {
      ...
      id: string;
      label: string;
      tokenType: "oauth" | "site" | "workspace";
      siteId?: string;
      workspaceId?: string;
      scopes?: string[];
    }
...
const WebflowTokenManager = (props: CustomSettingsComponentProps): JSXElement => {
  ...
  props.onMessage((msg: unknown) => {
    ...
    if (message.type === "tokenValidated") {
      ...
      setTokens((tkns) =>
        tkns.map((t) =>
          t.id === message.id ? { ...t, status: "invalid" as const, error: message.error } : t,
        ),
      );
      saveTokens();
        ...
        <div style="padding: 12px 16px;">
          ...
          >
            <Show
              ...
              fallback={hasValidToken() ? <>+ Add Another Account</> : <>🔗 Connect with Webflow</>}
            >
          ...
          <Show when={isAuthenticating()}>
            <div style="font-size: 11px; color: #888; text-align: center; margin-bottom: 12px;">
              This may take 10-30 seconds to open the browser.
              <br />
                ...
                <div style="flex: 1; min-width: 0;">
                  ...
                  <Show when={token.scopes && token.scopes.length > 0}>
                    <div style="font-size: 9px; color: #555; margin-top: 4px;">
                      Scopes: {token.scopes!.slice(0, 3).join(", ")}
                      {token.scopes!.length > 3 ? ` +${token.scopes!.length - 3} more` : ""}
                    </div>
                  </Show>
          ...
          <div style="font-size: 10px; color: #666; margin-top: 8px;">
            Get tokens from{" "}
            <a href="https://webflow.com/dashboard" target="_blank" style="color: #4dabf7;">
