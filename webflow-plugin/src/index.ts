...
export async function activate(api: PluginAPI): Promise<void> {
  ...
  const messageDisposable = api.settings.onMessage(async (message: unknown) => {
    ...
    if (msg.type === "validateToken" && msg.id && msg.token) {
      ...
      const { cwd, siteId, siteToken } = msg as {
        cwd?: string;
        siteId?: string;
        siteToken?: string;
      };
  ...
  const initMenuDisposable = api.contextMenu.registerItem(
    ...
    async (ctx) => {
      ...
      if (!validToken) {
        api.notifications.showError(
          "Not connected to Webflow. Open Settings → Webflow to connect.",
        );
        return;
      }
      ...
      try {
        ...
        if (sites.length === 0) {
          api.notifications.showError(
            "No Webflow sites found. Make sure your token has access to at least one site.",
          );
          return;
        }
  ...
  const initComponentsMenuDisposable = api.contextMenu.registerItem(
    ...
    async (ctx) => {
      ...
      const workspaceToken = tokens.find(
        (t) => t.status === "valid" && (t.type === "oauth" || t.type === "workspace"),
      );
...
`;
      ...
      api.notifications.showInfo(
        `Code Components library initialized in code-components folder! Click on webflow.json to manage.`,
      );
