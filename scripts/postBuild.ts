...
const BUILD_FOLDER = path.resolve(
  path.join(APP_BUNDLE_FOLDER, "Contents", "Resources", "app", "views", "ivde"),
);
...
cpSync("./vendor", path.join(APP_BUNDLE_FOLDER, "Contents", "MacOS", "vendor"), {
  recursive: true,
  dereference: true,
});
...
async function injectAnalyticsConfig() {
  ...
  try {
    ...
    console.log(
      `--------> Mixpanel token: ${analyticsConfig.mixpanelToken ? "***configured***" : "none"}`,
    );
