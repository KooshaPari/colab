...
async function vendorCmake() {
  ...
  if (platform === "darwin") {
    ...
    execSync(
      `mkdir -p vendors/cmake && curl -L ${cmakeUrl} | tar -xz --strip-components=1 -C vendors/cmake`,
      {},
    );
    ...
    execSync(
      `mkdir -p vendors/cmake && curl -L ${cmakeUrl} -o vendors/cmake.zip && powershell -ExecutionPolicy Bypass -Command Expand-Archive -Path vendors/cmake.zip -DestinationPath vendors/cmake-temp && mv vendors/cmake-temp/cmake-${cmakeVersion}-windows-${arch}/* vendors/cmake/`,
      {},
    );
...
async function vendorZig() {
  ...
  if (platform === "darwin") {
    execSync(
      `mkdir -p vendors/zig && curl -L https://ziglang.org/download/0.13.0/zig-macos-${arch}-0.13.0.tar.xz | tar -xJ --strip-components=1 -C vendors/zig zig-macos-${arch}-0.13.0/zig zig-macos-${arch}-0.13.0/lib zig-macos-${arch}-0.13.0/doc`,
      {},
    );
    console.log("--------> Zig vendored for macOS");
  } else if (platform === "linux") {
    execSync(
      `mkdir -p vendors/zig && curl -L https://ziglang.org/download/0.13.0/zig-linux-${arch}-0.13.0.tar.xz | tar -xJ --strip-components=1 -C vendors/zig zig-linux-${arch}-0.13.0/zig zig-linux-${arch}-0.13.0/lib zig-linux-${arch}-0.13.0/doc`,
      {},
    );
    ...
    execSync(
      `mkdir -p vendors/zig && curl -L https://ziglang.org/download/0.13.0/${zigFolder}.zip -o vendors/zig.zip && powershell -ExecutionPolicy Bypass -Command Expand-Archive -Path vendors/zig.zip -DestinationPath vendors/zig-temp && mv vendors/zig-temp/${zigFolder}/zig.exe vendors/zig && mv vendors/zig-temp/${zigFolder}/lib vendors/zig/`,
      {},
    );
