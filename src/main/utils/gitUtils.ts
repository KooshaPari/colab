...
export const gitStashCreate = async (
  repoRoot: string,
  message?: string,
  options: string[] = [],
) => {
...
export const gitStashShow = async (repoRoot: string, stashName: string) => {
  try {
    const stashContent = await git(repoRoot).stash([
      "show",
      "--name-status",
      "--include-untracked",
      stashName,
...
export const checkGitHubCredentials = async (): Promise<{
...
export const gitLogRemoteOnly = async (
  repoRoot: string,
  localBranch: string,
  remoteBranch: string,
) => {
  try {
    ...
    const result = await git(repoRoot).log([`${remoteBranch}`, `^${localBranch}`, "--name-status"]);
    return result;
  } catch (error) {
...
export const gitStageSpecificLines = async (
  repoRoot: string,
  filePath: string,
  startLine: number,
  endLine: number,
  ...
  try {
    ...
    for (const hunk of hunks) {
      ...
      console.log(
        `Checking hunk: new lines ${hunk.newStart}-${hunkEndLine}, looking for line ${startLine}`,
      );
      ...
      if (startLine >= hunk.newStart && startLine <= hunkEndLine) {
        ...
        if (hunk.newCount > 20) {
          console.log(
            `⚠️  Large hunk detected (${hunk.newCount} lines). Analyzing if this is pure reformatting...`,
          );
          ...
          console.log(
            `Content analysis: ${additions} additions, ${deletions} deletions, ${totalContentLines} total content changes out of ${hunk.lines.length} lines`,
          );

          // If more than 80% of the hunk is just formatting, refuse to stage
          if (totalContentLines < hunk.lines.length * 0.2) {
            throw new Error(
              `Cannot stage individual lines from a large reformatting hunk (${hunk.newCount} lines, mostly formatting). Please stage the entire file or revert the formatting changes first.`,
            );
          }

          console.log(
            `Large hunk contains significant content changes (${totalContentLines}/${hunk.lines.length} lines), proceeding with staging...`,
          );
        }
        break;
      }
    ...
    if (!targetHunk) {
      ...
      if (closestHunk) {
        console.log(
          `Using closest hunk: new lines ${closestHunk.newStart}-${closestHunk.newStart + closestHunk.newCount - 1} (distance: ${closestDistance})`,
        );

        // Also check the closest hunk for large size
        if (closestHunk.newCount > 20) {
          console.log(
            `⚠️  Large hunk detected (${closestHunk.newCount} lines). Analyzing if this is pure reformatting...`,
          );
          ...
          console.log(
            `Content analysis: ${additions} additions, ${deletions} deletions, ${totalContentLines} total content changes out of ${closestHunk.lines.length} lines`,
          );

          // If more than 80% of the hunk is just formatting, refuse to stage
          if (totalContentLines < closestHunk.lines.length * 0.2) {
            throw new Error(
              `Cannot stage individual lines from a large reformatting hunk (${closestHunk.newCount} lines, mostly formatting). Please stage the entire file or revert the formatting changes first.`,
            );
          }

          console.log(
            `Large hunk contains significant content changes (${totalContentLines}/${closestHunk.lines.length} lines), proceeding with staging...`,
          );
        }
    ...
    const patchLines = [...headers, targetHunk.header, ...targetHunk.lines];
...
export const gitTrackRemoteBranch = async (
  repoRoot: string,
  branchName: string,
...
export const gitCreatePatchFromLines = async (
  repoRoot: string,
  filePath: string,
  startLine: number,
  endLine: number,
