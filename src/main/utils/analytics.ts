...
if (
  (config.community.enabled || config.registered.enabled) &&
  MIXPANEL_TOKEN &&
  MIXPANEL_TOKEN !== "BUILD_TIME_MIXPANEL_TOKEN"
) {
...
export const track = {
  ...
  appLaunch: (props: { channel: string; appName: string; version: string; hash: string }) => {
  ...
  performanceMetric: (props: { metric: string; value: number; context?: string }) => {
    sendToMixpanel("performance_metric", props);
  },

  // Feature usage (maps to different event based on auth status)
  featureUsed: (props: { feature: string; action?: string; metadata?: any }) => {
  ...
  workspaceCreated: (props: { workspaceType: string; source: string }) => {
    if (isAuthenticated()) {
