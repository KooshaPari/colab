...
const openWorkspaceWindows = (workspace: CurrentDocumentTypes["workspaces"]) => {
...
const hideWorkspaceWindows = (workspace: CurrentDocumentTypes["workspaces"]) => {
...
type WindowConfigType = NonNullable<CurrentDocumentTypes["workspaces"]["windows"]>[0];
    ...
    handlers: {
      requests: {
        ...
        newPreviewNode: ({ candidateName }) => {
          const nodeName = getUniqueNewName(COLAB_PROJECTS_FOLDER || "", "new-project");
      ...
      messages: {
        ...
        addToken: ({ name, url, endpoint, token }) => {
          setTimeout(() => {
            const insertedToken = db.collection("tokens").insert({ name, url, endpoint, token });
...
function openBunnyWindow(screenX?: number, screenY?: number) {
  ...
  if (screenX != null && screenY != null) {
    ...
    x = workArea.x + Math.floor(Math.random() * Math.max(0, workArea.width - size));
    y = workArea.y + Math.floor(Math.random() * Math.max(0, workArea.height - size));
  }
