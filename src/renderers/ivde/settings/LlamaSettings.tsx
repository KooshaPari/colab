import { type JSXElement, createSignal, createEffect, onMount, batch, For, Show } from "solid-js";
...
export const LlamaSettings = (): JSXElement => {
  ...
  const [currentTemperature, setCurrentTemperature] = createSignal(
    state.appSettings.llama?.temperature || 0.1,
  );
          ...
          <SettingsPaneFormSection label="Status">
            <SettingsPaneField label="Models">
              <div style="background: #202020; padding: 8px; color: #d9d9d9; font-size: 12px; border-radius: 4px;">
                Status:{" "}
                <span style="background: #2b2b2b; padding: 4px 8px; margin-left: 8px; border-radius: 2px;">
                  {statusMessage()}
                </span>
              </div>
          ...
          <SettingsPaneFormSection label="Model Configuration">
            <SettingsPaneField label="Model">
              ...
              >
                {availableModels().length > 0 ? (
                  availableModels().map((model) => (
                    <option
                      value={model.name}
                      selected={model.name === state.appSettings.llama.model}
                    >
                      {model.name} ({model.source === "llama" ? "llama.cpp" : "legacy"},{" "}
              ...
              <div style="font-size: 11px; color: #999; margin-top: 4px;">
                The model to use for code completion.{" "}
                {availableModels().length === 0
                  ? "No models detected - install models below."
            ...
            <SettingsPaneField label="Temperature">
              <div style="display: flex; flex-direction: column; gap: 8px;">
                ...
                <div style="font-size: 11px; color: #999; margin-top: 4px;">
                  0.0-0.3: Focused (good for code) • 0.4-0.7: Balanced • 0.8-1.0: Creative • 1.0+:
                  Very random
                </div>
              </div>
            </SettingsPaneField>
                  ...
                  {(model) => {
                    ...
                    const isInstalled = () =>
                      availableModels().some(
                        (m) => m.source === "llama" && m.name === expectedModelName,
                      );
