...
class AICompletionService {
  ...
  async isModelAvailable(): Promise<boolean> {
    ...
    try {
      ...
      return (
        result.models?.some(
          (model: any) => model.name === modelName || model.path.includes(modelName),
        ) || false
      );
    } catch {
      return false;
  ...
  async getInlineCompletions(request: AICompletionRequest): Promise<AIInlineCompletionResponse> {
    ...
    try {
      ...
      if (
        lastWord.length === 1 &&
        commonNonCodeWords.some((word) => completion.toLowerCase().trim() === word.toLowerCase())
      ) {
        return { items: [] };
      }
