...
class GitHubService {
  ...
  async searchRepositories(
    ...
    const searchQuery = includeUserFilter
      ? `${query} user:${state.appSettings.github.username}`
      : query;
  ...
  > {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch repository branches: ${response.status} ${response.statusText}`,
      );
    }
