  ...
  return (
    <>
      ...
      <div style={{ "margin-bottom": "12px" }}>
        <For each={TEMPLATE_NODES}>{(template) => <TemplateNodeItem template={template} />}</For>
      </div>
    </>
  );
};
      ...
      <Show when={isExpanded()}>
        <TreeUL showLeftBar={true}>
          ...
          <For each={getFilteredNodeChildren(node)}>
            {(childNode) => (childNode ? <FileTree node={childNode} readonly={readonly} /> : null)}
          </For>
            ...
            <TreeUL>
              ...
              <TreeLI node={null}>
                <div
                  ...
                  onClick={() => {
                    setNumResultsToShow(numResultsToShow() + numResultsHidden());
                  }}
                >
...
const TreeLI = ({
  ...
  const isFolderDropTarget = () => {
    return state.dragState?.type === "node" && state.dragState.targetFolderPath === node.path;
  };
  ...
  const onLiContextMenu = (e: MouseEvent) => {
    ...
    const showContextMenu = async () => {
      const openTabs = Object.values(win.tabs).filter((tab) => {
        ...
        return (
          tabNode &&
          getProjectForNode(tabNode)?.id === getProjectForNode(_nodeToRender)?.id &&
      ...
      try {
        const pluginMenuItems = await electrobun.rpc?.request.pluginGetContextMenuItems({
          context: "fileTree",
        });
      ...
      const projectForRoot =
        nodeIsProjectRoot && _nodeToRender?.path ? getProjectByRootPath(_nodeToRender.path) : null;
  ...
  return (
        ...
        if (dragState && dragState.type === "node") {
          ...
          if (_node.type === "dir") {
            ...
            const isNotDescendant =
              dragState.isTemplate || !isDescendantPath(dragState.nodePath, _node.path);
    ...
    >
      ...
      >
        ...
        >
          <Show when={getFilteredNodeChildren(nodeToRender())?.length || showChildPreview()}>
            <img
              width={10}
              height={10}
      ...
      >
        ...
        <Show when={!readonly}>
          <div style="position: absolute; display: flex; top: 0px; right: 0px; left: 0px; height: 23px; align-items: center; justify-content: right;">
            <Show when={isHovered()}>
              <FileTreeItemControlButton label="..." onClick={editNodeSettings} />
            </Show>
          </div>
        </Show>
        <div
    ...
    <>
      ...
      <For each={projectsWithResultsAsArray()}>
        {(project) => {
          ...
          return (
            <TreeUL>
              <FindAllResultProjectTree projectId={project.id} projectNode={_projectNode} />
            </TreeUL>
          );
        }}
      </For>
        ...
        <TreeUL showLeftBar={true}>
          <For each={getSearchResults()}>
            {(childPath) => {
              ...
              return childNode ? (
                <FileTree node={childNode} readonly={true} projectId={projectId} />
              ) : null;
            }}
          </For>
        </TreeUL>
