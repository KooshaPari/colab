...
export class TmateCommandAdapter implements TmateAdapter {
  async startShare(terminalId: string): Promise<{ sshCommand: string; webUrl?: string }> {
    const result = await execCommand("tmate", ["-S", `/tmp/${terminalId}.sock`, "new-session", "-d"]);
    if (result.code !== 0) throw new Error(`tmate start share failed: ${result.stderr}`);

    const sshInfo = await execCommand("tmate", ["-S", `/tmp/${terminalId}.sock`, "display", "-p", "#tmate_ssh"]);
    const webInfo = await execCommand("tmate", ["-S", `/tmp/${terminalId}.sock`, "display", "-p", "#tmate_web"]);
